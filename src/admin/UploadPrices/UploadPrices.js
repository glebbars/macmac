...
const UploadPrices = () => {
  ...
  const updateObjPricesWithExcel = (e) => {
    readXlsxFile(e.target.files[0], { sheet: 1 })
      .then((rows) => {
        ...
        rows.forEach((row) => {
          if (
            ...
            const formattedPrice =
              Math.round(Math.ceil(row[1] * usdExchangeRate) / 5) * 5;
  ...
  return (
    <label className="admin__upload-file">
      <input
        className="admin__upload-file__input"
        type="file"
        onChange={(e) => updateObjPricesWithExcel(e)}
      />
