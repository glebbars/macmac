...
const ProductsPageSizeLength = ({ sortedProductsLength }) => {
  ...
  useEffect(() => {
    if (sortedProductsLength > pageSize) {
      if (pageNum * 24 > sortedProductsLength) {
        setCurrentArrLength(
          `${(pageNum - 1) * 24 + 1}-${sortedProductsLength}`
        );
      } else {
  ...
  const versionOfResultWord =
    String(sortedProductsLength).slice(-1) === "1"
      ? "результата"
      : "результатов";

  return (
    <div className="products__page-size__text-length">
      {currentArrLength} из {sortedProductsLength} {versionOfResultWord}
    </div>
  );
};
